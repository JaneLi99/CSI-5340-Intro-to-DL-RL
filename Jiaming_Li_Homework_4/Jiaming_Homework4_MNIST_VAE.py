...
def display_images(in_, out, n = 1, label=None, count=False):
    for N in range(n):
        if in_ is not None:
            ...
            for i in range(16):
                plt.subplot(2, 8, i + 1)
                plt.imshow(in_pic[i + 4 * N], cmap='gray', interpolation='none')
                plt.axis('off')
        out_pic = out.data.cpu().view(-1, 28, 28)
        plt.figure(figsize=(18, 4))
        for i in range(16):
            plt.subplot(2, 8, i + 1)
            plt.imshow(out_pic[i + 4 * N], cmap='gray', interpolation='none')
            plt.axis('off')
...
def training_model(num_epochs):
    ...
    KLD_train_loss_list = []
    KLD_test_loss_list = []
    train_dataset, test_dataset, train_loader, test_loader = get_dataset_MNIST(batch_size)
    ...
    display_images(y, y_hat, 1, f'Epoch {epoch}')
...
if __name__ == "__main__":
    ...
    plt.title('Train and Test Loss of MNIST Dataset (VAE)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    plt.title('KLD Train and Test Loss of MNIST Dataset (VAE)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend(loc = "upper right")
    plt.show()
    ...
    plt.plot(epochs, BCE_test_loss_list, color = 'orange', label = 'B Test Loss')
    plt.title('KLD Train and Test Loss of MNIST Dataset (VAE)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend(loc = "upper right")
