...
def training_model(num_epochs, train_loader):
    ...
    for epoch in range(num_epochs):
        ...
        print(f'[Epoch]: {epoch + 1} [Generator Loss]: {Generator_losses:.4f} '
              f'[Discriminator Loss]: {Discriminator_losses:.4f} [JSD Loss]: {jsd_loss:.4f}')
...
if __name__ == "__main__":
    ...
    Generator = Generator(g_input_dim = input_dim, g_output_dim = mnist_dim).to(device)
    ...
    plt.title('Generator and Discriminator Loss of MNIST Dataset (GAN)')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    plt.title('JSD of MNIST Dataset (GAN)')
    plt.xlabel('Epoch')
